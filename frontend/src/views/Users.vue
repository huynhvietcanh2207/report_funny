  ...
  <div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
      <div>
        ...
        <p class="text-slate-500 dark:text-slate-400">Danh sách tất cả nhân viên trong hệ thống TeamVoice AI</p>
      </div>
      <button @click="openModal()" class="bg-gradient-to-r from-[#FD94B4] to-[#F34455] text-white px-5 py-2.5 rounded-xl font-bold shadow-[0_0_15px_rgba(243,68,85,0.4)] hover:shadow-[0_0_20px_rgba(243,68,85,0.6)] hover:-translate-y-0.5 transition-all flex items-center gap-2 active:scale-95">
    ...
    <!-- User Table -->
    <div class="glass-card bg-white dark:bg-[#131d1a] border-black/5 dark:border-[#FD94B4]/10 rounded-3xl overflow-hidden shadow-xl">
            ...
            <tr v-for="user in userStore.users" :key="user.id" class="hover:bg-slate-50 dark:hover:bg-white/5 transition-colors group">
              ...
              <td class="px-6 py-4 text-right">
                <div class="flex items-center justify-end gap-2">
                  ...
                  <button @click="openModal(user)" class="p-2.5 bg-slate-100 dark:bg-white/5 hover:bg-[#F34455]/10 text-slate-500 hover:text-[#F34455] dark:hover:text-[#FD94B4] rounded-xl transition-all border border-transparent hover:border-[#F34455]/20" title="Chỉnh sửa">
                    <Edit2 class="w-4 h-4" />
                  </button>
                  <button @click="handleDelete(user.id)" class="p-2.5 bg-slate-100 dark:bg-white/5 hover:bg-rose-500/10 text-slate-500 hover:text-rose-500 rounded-xl transition-all border border-transparent hover:border-rose-500/20" title="Xóa">
    ...
    <!-- User Detail Modal -->
    <div v-if="showDetailModal" class="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/60 backdrop-blur-md animate-fade-in">
       <div class="glass-card bg-white dark:bg-[#131d1a] border-black/10 dark:border-white/10 w-full max-w-2xl rounded-[3rem] p-0 overflow-hidden relative shadow-2xl animate-scale-in">
          <!-- Header Image/Banner -->
          ...
          <div class="px-8 pb-8 -mt-12 relative">
             <div class="flex flex-col sm:flex-row items-center sm:items-end gap-6 mb-8">
                ...
                <div class="text-center sm:text-left flex-1">
                   <h2 class="text-3xl font-black text-slate-800 dark:text-white tracking-tight leading-none mb-1">{{ selectedUser?.name }}</h2>
                   <p class="text-slate-500 font-bold">{{ selectedUser?.email }}</p>
                </div>
                <button @click="openModal(selectedUser); showDetailModal = false" class="px-5 py-2.5 bg-slate-100 dark:bg-white/5 hover:bg-[#F34455]/10 text-slate-600 dark:text-white rounded-2xl text-sm font-black uppercase tracking-widest border border-black/5 dark:border-white/10 transition-all flex items-center gap-2">
    ...
    <!-- Add/Edit Modal -->
    <div v-if="showModal" class="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/60 backdrop-blur-md animate-fade-in">
       <div class="glass-card bg-white dark:bg-[#131d1a] border-black/10 dark:border-[#FD94B4]/20 w-full max-w-md rounded-[3rem] p-8 relative shadow-2xl animate-scale-in">
...
import { UserPlus, Edit2, Trash2, X, Search, Filter, Eye, Mail, Calendar, Shield, LayoutGrid, ChevronRight } from 'lucide-vue-next';
