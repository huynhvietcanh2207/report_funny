<template>
  <div class="max-w-4xl mx-auto space-y-8">
    <!-- Profile Header Card -->
    <div class="glass-card bg-white dark:bg-[#131d1a] border-black/5 dark:border-[#FD94B4]/10 rounded-2xl sm:rounded-[3rem] p-5 sm:p-8 lg:p-12 relative overflow-hidden shadow-2xl">
      ...
      <div class="md:hidden absolute top-4 right-4 z-20">
        <button @click="showMobileMenu = !showMobileMenu" class="p-2 text-slate-400 hover:text-slate-800 dark:hover:text-white bg-slate-50/50 dark:bg-white/5 rounded-xl transition-colors shadow-sm border border-black/5 dark:border-white/5">
          <MoreVertical class="w-5 h-5" />
        </button>
        
        <div v-if="showMobileMenu" class="absolute right-0 top-full mt-2 w-48 bg-white dark:bg-[#131d1a] border border-black/10 dark:border-white/10 rounded-2xl shadow-2xl overflow-hidden animate-fade-in z-50 py-2">
          <button @click="showEditModal = true; showMobileMenu = false" class="w-full text-left px-4 py-3 text-[11px] font-black uppercase tracking-widest text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-white/5 flex items-center gap-3 transition-colors">
            <Edit2 class="w-4 h-4 text-[#FD94B4]" /> Cập nhật hồ sơ
          </button>
        </div>
      </div>

      <div class="flex flex-col md:flex-row items-center gap-8 relative z-10">
        <div class="w-20 h-20 sm:w-32 sm:h-32 rounded-2xl sm:rounded-[2.5rem] bg-gradient-to-tr from-[#FD94B4] to-[#F34455] flex items-center justify-center text-white text-3xl sm:text-5xl font-black shadow-[0_0_40px_rgba(243,68,85,0.4)] animate-pulse-slow">
          {{ authStore.user?.name.charAt(0).toUpperCase() }}
        </div>
        
        <div class="text-center md:text-left flex-1">
          <div class="flex flex-col md:flex-row md:items-center gap-3 mb-2">
            <h1 class="text-2xl sm:text-4xl font-black text-slate-800 dark:text-white tracking-tight">{{ authStore.user?.name }}</h1>
          ...
          <p class="text-slate-500 dark:text-slate-400 text-sm sm:text-lg mb-4 sm:mb-6 font-medium">{{ authStore.user?.email }}</p>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-8 pb-20">
      <!-- Edit Profile (Desktop) -->
      <div class="hidden md:block glass-card bg-white dark:bg-[#131d1a] border-black/5 dark:border-[#FD94B4]/10 rounded-2xl sm:rounded-[2.5rem] p-5 sm:p-8 shadow-xl">
      ...
      <div class="space-y-8">
         <div class="glass-card bg-white dark:bg-[#131d1a] border-black/5 dark:border-[#FD94B4]/10 rounded-2xl sm:rounded-[2.5rem] p-5 sm:p-8 shadow-xl">
         ...
         <div class="glass-card bg-white dark:bg-[#131d1a] border-rose-500/20 rounded-2xl sm:rounded-[2.5rem] p-5 sm:p-8 shadow-xl border-dashed">
    ...
    <div v-if="showEditModal" class="md:hidden fixed inset-0 z-[100] flex items-end bg-black/60 backdrop-blur-md animate-fade-in">
       <div class="glass-card bg-white dark:bg-[#131d1a] border-black/10 dark:border-white/10 w-full rounded-t-[2rem] p-6 relative shadow-2xl animate-slide-up-drawer overflow-y-auto max-h-[85vh] custom-scrollbar">
          <button @click="showEditModal = false" class="absolute top-6 right-6 p-2 hover:bg-slate-100 dark:hover:bg-white/10 rounded-xl text-slate-400 hover:text-slate-800 dark:hover:text-white transition-all z-10">
            <X class="w-5 h-5" />
          </button>
          
          <h2 class="text-xl font-black text-slate-800 dark:text-white mb-6 flex items-center gap-3 pr-10">
            <div class="w-10 h-10 shrink-0 rounded-xl bg-[#F34455]/10 flex items-center justify-center text-[#F34455]">
              <User class="w-5 h-5" />
            </div>
            Cập nhật hồ sơ
          </h2>
          
          <form @submit.prevent="() => { handleUpdate(); showEditModal = false; }" class="space-y-6 pb-6">
            <div>
              <label class="block text-[10px] font-black uppercase tracking-widest text-slate-400 dark:text-slate-500 mb-2 ml-1">Họ và tên</label>
              <input v-model="form.name" type="text" required
                     class="w-full bg-slate-50 dark:bg-[#0c1210] border border-black/5 dark:border-white/5 rounded-2xl px-5 py-4 text-slate-800 dark:text-white outline-none focus:border-[#F34455]/50 transition-all font-bold shadow-inner" />
            ...
            <button type="submit" :disabled="loading"
                    class="w-full py-4 bg-gradient-to-r from-[#FD94B4] to-[#F34455] text-white rounded-2xl font-black uppercase tracking-widest text-[10px] shadow-[0_10px_20px_rgba(243,68,85,0.3)] hover:shadow-[0_15px_30px_rgba(243,68,85,0.5)] active:scale-95 disabled:opacity-50 mt-4 transition-all">
              {{ loading ? 'Đang xử lý...' : 'Lưu thay đổi hồ sơ' }}
            </button>
          </form>
       </div>
...
import { User, Calendar, ShieldCheck, Mail, Lock, MoreVertical, X, Edit2 } from 'lucide-vue-next';
...
const closeMenu = (e) => {
  if (showMobileMenu.value && !e.target.closest('.md\\:hidden')) {
